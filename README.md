...
* [`src/course_store/styles.cljs`](src/course_store/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/course_store/`](src/course_store/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/course_store/core.cljs): contains the SPA entry point, `init`
* [`test/course_store/`](test/course_store/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `course-store` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/course_store/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns course-store.example
  (:require [course-store.config :as config])
